  ...
  return (
    <section className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 sm:pt-32 pb-20">
      <div className="text-center mb-10">
        ...
        <p className="text-xs text-[#5C574F]">Enter your order number along with the email or phone used at checkout.</p>
      </div>

      {verifying && (
        <div className="mb-6 p-3 bg-[#B8945A]/10 border border-[#B8945A]/40 text-[#1E1C1A] text-xs text-center">
      ...
      <form onSubmit={handleSubmit} className="bg-[#FFFDF9] border border-[#E8DDCD] p-6 sm:p-8 mb-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <input
          ...
          placeholder="Order Number (AUR-123456)"
          className="px-3 py-2.5 bg-[#FFFDF9] border border-[#E8DDCD] text-sm text-[#1E1C1A] focus:outline-none focus:border-[#B8945A] sm:col-span-1"
        />
        <input
          type="email"
          name="email"
        ...
        <button
          type="submit"
          disabled={loading}
          className="sm:col-span-3 py-3 bg-[#1E1C1A] hover:bg-[#B8945A] text-white text-xs font-semibold tracking-[0.2em] uppercase transition-colors disabled:opacity-50"
        >
          {loading ? 'Searching...' : 'Track Order'}
        </button>
      </form>

      {error && <div className="p-3 bg-red-50 border border-red-200 text-red-700 text-xs mb-8 text-center">{error}</div>}
        ...
        <div className="bg-[#FFFDF9] border border-[#E8DDCD] p-6 sm:p-8">
          <div className="flex items-center justify-between flex-wrap gap-2 mb-6 pb-6 border-b border-[#E8DDCD]">
            <div>
              ...
              <p className="text-xs text-[#5C574F]">Placed on {new Date(order.created_at).toLocaleDateString('en-IN')}</p>
            </div>
            <span className="text-xs uppercase tracking-wider font-bold text-white bg-[#B8945A] px-3 py-1.5">
            ...
            {(order.timeline || []).map((event, idx) => {
              ...
              return (
                <div key={idx} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#1E1C1A]">{event.status_display}</p>
                    {event.note && <p className="text-xs text-[#5C574F]">{event.note}</p>}
                    <p className="text-[10px] text-[#5C574F]/70">{new Date(event.created_at).toLocaleString('en-IN')}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="space-y-2 pt-4 border-t border-[#E8DDCD] mb-4">
            {order.items.map((item) => (
              <div key={item.id} className="flex justify-between text-xs text-[#1E1C1A]">
